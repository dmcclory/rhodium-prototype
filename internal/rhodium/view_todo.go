		...
		},
		{
			...
			Action: func() tea.Cmd {
				...
				if _, cached := a.cache.prComments[brain.PRKey(it.pr.Repo, it.pr.Number)]; !cached {
					return tea.Batch(loadCommentsCmd(it.pr), a.openComments(router.RouteTodo))
				}
				return a.openComments(router.RouteTodo)
			},
		},
	}
}
...
func (v *todoView) rebuild(a *app) {
	...
	cols := computeTodoCols(append(append([]todoItem{}, actionable...), newPRs...))
...
type todoItem struct {
	...
	notes     int
	cols      prCols // populated by todoView.rebuild for column alignment
}
...
func (i todoItem) Title() string {
	...
	b.WriteString("  ")
	b.WriteString(padRight(truncateDisplay(i.pr.Title, prMaxTitleWidth), i.cols.title))
	b.WriteString("  ")
...
func computeTodoCols(items []todoItem) prCols {
	prs := make([]prItem, len(items))
	for i, it := range items {
		prs[i] = prItem{pr: it.pr}
	}
	cols := computePRCols(prs)
	cols.anyScrutiny = false // todo view doesn't render the [S] prefix
	return cols
}
